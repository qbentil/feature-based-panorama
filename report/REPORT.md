...
Four folders live in `data/scenes/`. They were generated by `scripts/generate_sample_images.py` as a controlled textured mural (labelled buildings and indoor posters) so overlap, lighting, and viewpoint can be varied on demand. The same code accepts phone photographs dropped into the same folders.

| Set | Images | Role |
|---|---|---|
| `scene_main` | 5 overlapping courtyard views, about 50% overlap | Primary panorama |
| `scene_viewpoint` | Front, oblique, and side of the same region | Viewpoint change |
| `scene_lighting` | Daylight, shade, and dim of one view | Illumination photographs |
| `scene_second` | 5 overlapping noticeboard views | Extra scene in the app |
